...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    let prompt = "";
    if (customPrompt) {
      if (customPrompt.includes("{TEXTO}") || customPrompt.includes("{texto}")) {
        prompt = customPrompt.replace(/\{TEXTO\}|\{texto\}/g, rawText);
      } else {
        prompt = `${customPrompt}\n\nTexto a mejorar:\n"""${rawText}"""`;
      }
    } else {
...
7. Devuelve ÚNICAMENTE el texto final resultante. NO agregues comillas, ni títulos introductorios innecesarios, ni frases como "Aquí tienes el texto corregido:".
